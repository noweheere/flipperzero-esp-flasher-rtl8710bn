#pragma once

#include <furi.h>
#include <furi_hal.h>

#ifdef __cplusplus
extern "C" {
#endif

#define RX_BUF_SIZE (1024)
#define BAUDRATE    (115200)
#define FAST_BAUDRATE (921600)

typedef struct EspFlasherUart EspFlasherUart;
typedef void (*EspFlasherUartRxCallback)(uint8_t* buf, size_t len, void* context);

EspFlasherUart* esp_flasher_usart_init(void* context);
void esp_flasher_uart_free(EspFlasherUart* uart);
void esp_flasher_uart_set_handle_rx_data_cb(EspFlasherUart* uart, EspFlasherUartRxCallback callback);
void esp_flasher_uart_tx(EspFlasherUart* uart, uint8_t* data, size_t len);
void esp_flasher_uart_set_br(EspFlasherUart* uart, uint32_t br);

#ifdef __cplusplus
}
#endif